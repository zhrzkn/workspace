...
void Task1_Handler(void const * argument)
{
  ...
  for(;;)
  {
		sprintf(myMessage, "State of Led: %d  \r\n",
				HAL_GPIO_ReadPin(GPIOD,GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15 ));
		 HAL_UART_Transmit(&huart2, myMessage, strlen(myMessage), 1);

		osDelay(1000);       	// vTaskDelay(pdMS_TO_TICKS(1000));
  }
  /* USER CODE END 5 */
}
